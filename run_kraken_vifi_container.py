...
def call_fastvifi_pipeline(args):
    ...
    docker_image_tag = "sarajava/fastvifi:v1.2.3"
    # Command for Singularity
    if args.singularity:
        # Running as a user
        singularity_image_tag = "fastvifi_v1.2.3.sif"
        ...
        command += "--bind {}:/home/kraken2-db ".format(kraken_db_path) + \
            ...
            "--bind {}/run_alignment_filter.sh:/home/fastvifi/run_alignment_filter.sh ".format(os.getcwd()) + \
    ...
    # Command for Docker
        ...
        command += "--read-only -v {}:/home/kraken2-db ".format(kraken_db_path) + \
            ...
            "-v {}/run_alignment_filter.sh:/home/fastvifi/run_alignment_filter.sh ".format(os.getcwd()) + \
            "-v {}/filter_reads_bwa_efficient.py:/home/fastvifi/filter_reads_bwa_efficient.py ".format(os.getcwd()) + \
    ...
    if args.keep_intermediate_files:
        command += " --keep-intermediate-files"
    if args.skip_bwa_filter:
        command += " --skip-bwa-filter "
    if args.memory_efficient_bwa_filter:
        command += " --memory-efficient-bwa-filter"
    if args.threads is not None:
        command += " --threads {} ".format(args.threads)
    if args.samtools_mem is not None:
        command += " --samtools-mem {} ".format(args.samtools_mem)
